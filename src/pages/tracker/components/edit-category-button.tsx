...
import { EditCategory } from "utilities/api";
...
function EditCategoryButton({ id, name, icon }: EditCategoryButtonProps) {
	...
	const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		if (await EditCategory(id, newName, newIcon)) {
			setCategoryExpenses((prev) => prev.map((category) => (category.id === id ? { ...category, name: newName, icon: newIcon } : category)));
		}
	...
	return (
		<>
			{isModalOpen !== 0 && (
				<div data-open={isModalOpen} className="fixed flex justify-center items-center w-full h-dvh top-0 left-0 z-10 bg-gray-500/30 backdrop-blur-sm data-[open='-1']:animate-fade-out data-[open='1']:animate-fade-in">
					<div data-offset={offset} className="max-w-[400px] w-full p-4 data-[offset='true']:pt-52 transition-all">
						<form onSubmit={onSubmit} className="flex flex-col gap-4 w-full p-4 rounded-md bg-white text-black font-medium">
							<div className="relative">
								...
								{/* <button className="absolute top-1/2 right-0 -translate-y-1/2 text-white p-2 bg-red-500 rounded-full">
									<FaTrash />
								</button> */}
							</div>
							<div className="relative flex flex-grow gap-4">
								<IconSelector icon={newIcon} setIcon={setNewIcon} setOffset={setOffset} />
								<input value={newName} onChange={(event) => setNewName(event.target.value)} id="name" placeholder="Name" required className="flex-grow p-2 border border-gray-500 rounded-md" />
							</div>
