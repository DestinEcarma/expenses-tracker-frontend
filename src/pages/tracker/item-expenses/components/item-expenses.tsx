import { ItemExpensesContext } from "../utilities/item-expenses-context";
...
const TIME: { hour12: true; hour: "2-digit"; minute: "2-digit" } = { hour12: true, hour: "2-digit", minute: "2-digit" };

function ItemExpenses() {
	const itemExpenses = useContext(ItemExpensesContext);
			...
			{itemExpenses
				...
				.map((item, index) => (
					<div className="flex gap-4 px-4 last:mb-4">
						<div className="flex flex-col justify-center items-center aspect-square h-20 text-gray-300 font-bold rounded-full border-8 border-gray-300 drop-shadow-md">
						...
						<div className="relative flex-grow flex justify-between gap-4">
							...
							{index !== itemExpenses.length - 1 && <hr className="absolute -bottom-2 border-1 border-gray-500 w-full" />}
						</div>
					</div>
				))}
		</div>
